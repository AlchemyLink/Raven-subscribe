...
Поддерживает **VLESS, VMess, Trojan, Shadowsocks, Hysteria2**, транспорты **XHTTP/SplitHTTP, WebSocket, gRPC, REALITY**, отдаёт конфиги в форматах Xray JSON, sing-box JSON и share-ссылках. Создан для **обхода цензуры**: экстренная ротация на резервные inbound-ы одним вызовом, персональный **резервный канал Hysteria2**, анти-DPI тюнинг клиента (ротация соединений xmux + padding для XHTTP) и эндпоинт метрик Prometheus для мониторинга.
...
- [Вывод и перенаправление inbound-ов](#вывод-и-перенаправление-inbound-ов)
- [Анти-DPI тюнинг клиента](#анти-dpi-тюнинг-клиента)
...
- **Экстренная ротация и killswitch** — один API-вызов переключает все подписки на резервные inbound-ы во время блокировки; обратно так же быстро
- **Вывод или перенаправление транспортов без удаления** — `exclude_inbound_tags` убирает мёртвый транспорт из всех подписок; `fallback_inbound_tags` переносит его из основной подписки в резервную
- **Персональный резерв Hysteria2** — каждому пользователю можно выдать запасной QUIC-канал (`/sub/{token}/hy2`) с индивидуальным вкл/выкл, валидацию делает нативный hysteria-демон на лету
- **Метрики Prometheus** — отдельный listener `/metrics` отдаёт gauge'и состояния fallback и счётчик отклонённых запросов для алертов

### Обход цензуры
- **Анти-DPI тюнинг клиента** — в XHTTP-конфиги добавляется ротация соединений `xmux` и случайный `xPaddingBytes`, корректно вложенные в `extra`, чтобы они реально доходили до диалера; `xmux` также пишется в `vless://` share-ссылки
- **Резервная подписка** — параллельный персональный fallback-токен + глобальный killswitch, рассчитанные на волны блокировок DPI/ТСПУ
- **DNS-сплит под РФ** — резолвить разрешённые локальные сервисы через отечественный резолвер (закрывает VPN-detection по geo-mismatch), а заблокированные домены оставлять на зарубежном DNS через прокси
...
#### Мониторинг, вывод транспортов и резерв

| Поле | По умолчанию | Описание |
|------|--------------|----------|
| `metrics_listen` | *(не задавать)* | Если задан (например `"127.0.0.1:9091"`), отдаёт эндпоинт Prometheus `/metrics` на **отдельном** listener — никогда не на публичном роутере подписок. См. [Мониторинг](#мониторинг-prometheus). |
| `fallback_inbound_tags` | `[]` | Теги inbound-ов, **переносимые** из основной подписки в персональную резервную (`/sub/fallback/*`). Тег остаётся живым на сервере; клиенты получают его только по своей fallback-ссылке. Держит транспорт в резерве за killswitch. |
| `exclude_inbound_tags` | `[]` | Теги inbound-ов, **убираемые из всех** ответов подписки (основная, резервная, JSON, links), при этом inbound остаётся живым на сервере. Чтобы вывести из клиентских загрузок транспорт, переставший проходить DPI, не удаляя его — восстановление мгновенно очисткой списка. **Клиенты должны перезапросить подписку.** |
| `killswitch_inbound_tags` | `[]` | Теги, которые глобальный killswitch добавляет/убирает на сервере через gRPC при переключении. Тег в списке = управляется killswitch (не слушает, пока выключатель выключен). |
| `killswitch_reconcile_interval_seconds` | `30` | Как часто reconcile-цикл killswitch переприменяет нужное состояние inbound-ов (самовосстановление после рестарта Xray). |
| `hysteria` | *(не задавать)* | Персональный резервный канал Hysteria2. См. [Персональный резерв Hysteria2](#персональный-резерв-hysteria2-нативный). |
...
| `/sub/{token}/hy2` | **Персональный резерв Hysteria2** — URI (`hysteria2://`) нативного резервного канала, отдельно от обнаружения через sing-box. Есть вариант `.b64`. См. [ниже](#персональный-резерв-hysteria2-нативный). |
...
Эндпоинт сделан, чтобы поднять громко **класс молчаливых сбоев**:
пользователь добавлен в БД, выглядит здоровым в админ-UI, но в running
xray-конфиге его UUID не появился — VPN-подключение отклоняется с
`proxy/vless/encoding: invalid request user id`. Типичные причины:

- `/etc/xray/config.d/` принадлежит root вместо `xrayuser` →
  `SyncDBToConfig` не может записать `*.raven.tmp` → drift копится
  каждую минуту (каждая попытка логирует WARN, но WARN никто не читает).
- Сбой gRPC `HandlerService.AlterInbound`.

**Два сигнала:**
- **`probe_ok`**: при старте демон пишет/читает/удаляет пробный файл
  в `config_dir`. EACCES → `probe_ok=false` + ERROR в логе main.go.
  Ловит регрессию прав на старте, а не при первом добавлении юзера.
- **`drift[]`**: список пар `{username, inbound_tag}`, где в DB юзер
  есть, а в on-disk конфиге его нет. Пустой → синхронизация чистая.

`Raven Dashboard` опрашивает этот эндпоинт и показывает:
- Постоянную плашку светофора на странице Settings.
- Inline-предупреждение в потоке "Add User" (через 3 сек проверяет —
  если новый юзер попал в drift, выдаёт ошибку админу).
...
## Вывод и перенаправление inbound-ов

Когда транспорт перестаёт проходить DPI (например, REALITY начинают детектить по поведению на первом хопе), обычно не нужно *удалять* серверный inbound — нужно убрать его из того, что скачивают клиенты, с мгновенным откатом. Это делают два параметра конфига без вмешательства в Xray:

| Рычаг | Эффект | Когда использовать |
|---|---|---|
| `fallback_inbound_tags` | **Переносит** тег из основной подписки в персональную резервную (`/sub/fallback/*`). Остаётся живым на сервере. | Держать транспорт в резерве за killswitch — клиенты достают его только по fallback-ссылке. |
| `exclude_inbound_tags` | **Убирает** тег из *всех* ответов подписки (основная, резервная, JSON, links). Остаётся живым на сервере. | Вывести мёртвый транспорт из клиентских загрузок, сохранив возможность мгновенно вернуть. |

Оба применяются централизованно ко всем трём обработчикам подписки. **Клиенты должны перезапросить подписку**, чтобы изменение вступило в силу. Для восстановления — уберите тег из списка, серверные inbound-ы менять не нужно.

Глобальный **killswitch резерва** (переключается из [Raven Dashboard](https://github.com/AlchemyLink/raven-dashboard) или в БД) разом включает/выключает все `/sub/fallback/*` URL, а для тегов из `killswitch_inbound_tags` добавляет/убирает listener на сервере через gRPC. Reconcile-цикл (`killswitch_reconcile_interval_seconds`) переприменяет нужное состояние, чтобы рестарт Xray не оставил его применённым наполовину.

---

## Анти-DPI тюнинг клиента

В генерируемые конфиги входит клиентский тюнинг против современного волюметрического / fingerprint-DPI (как на ТСПУ в РФ, net4people [#490](https://github.com/net4people/bbs/issues/490)/[#546](https://github.com/net4people/bbs/issues/546)):

- **Ротация соединений `xmux`** — XHTTP-outbound-ы получают настроенный `xmux` (ограниченное переиспользование + случайное время жизни соединения), чтобы один долгоживущий поток не светился всю сессию. Он вкладывается **внутрь** `streamSettings.xhttpSettings.extra`, потому что Xray пересобирает конфиг XHTTP из `extra` при его наличии и молча отбрасывает поля-сиблинги — `xmux` на верхнем уровне (как в старых версиях) оставался неактивным на каждом клиенте. `xmux` также пишется в `vless://` share-ссылки, чтобы его получали и URI-клиенты (Happ, v2box, v2rayN/v2rayNG).
- **Padding `xPaddingBytes`** — тело XHTTP-запросов/ответов дополняется до случайной длины (`"100-1000"` по умолчанию), выравнивая распределение размеров пакетов, чтобы DPI не мог за него зацепиться. Значение с сервера сохраняется; иначе подставляется дефолт.

Приоритет для обоих: верхний уровень сервера → вложенное в `extra` на сервере → встроенный дефолт. **Клиенты должны перезапросить подписку**, чтобы подхватить активный тюнинг; на живые соединения это не влияет до перезапроса.

---

## Мониторинг (Prometheus)

Задайте `metrics_listen` (например `"127.0.0.1:9091"`), чтобы поднять эндпоинт `/metrics` в текстовом формате Prometheus на **отдельном** listener. Он намеренно держится вне основного роутера, который публично доступен через vhost подписок — биндите на loopback и скрейпьте локально или по приватной сети.
...
- **`raven_fallback_enabled`** — алерт, если держится `0` вне планового окна обслуживания (резервная подписка в норме должна быть включена).
- **`raven_fallback_denied_total`** — считает пользовательские `403` от выключенного резерва, т.е. реальное влияние на клиентов во время инцидента.
...
**Важно:** обнаруженные через sing-box Hysteria2-пользователи исключаются из Xray JSON подписок (`/sub/{token}`, `/c/{token}`) — Xray-core не умеет Hysteria2. Они отдаются через специальные Hysteria2-эндпоинты ниже. (Отдельный нативный [персональный резерв](#персональный-резерв-hysteria2-нативный) может опционально ехать в link-подписке через `in_main_sub`.)
...
### Персональный резерв Hysteria2 (нативный)

Отдельно от обнаружения через sing-box выше, Raven может выдать каждому пользователю **запасной QUIC-канал** на нативный hysteria-демон — полезно как второй транспорт, когда основной TCP/XHTTP-путь под волной блокировок. Управляется блоком `hysteria` в конфиге (по умолчанию выключен):
...
| Поле | Описание |
|---|---|
| `enabled` | Главный выключатель резервного канала. |
| `host` / `port` | Адрес, куда подключаются клиенты (например, UDP-порт relay, проброшенный на hysteria-сервер). |
| `obfs_type` / `obfs_password` | Общая (на весь сервер) обфускация Salamander/Gecko. |
| `sni` | TLS SNI, предъявляемый клиентом (совпадает с CN серверного сертификата). |
| `cert_pin` | Опциональный SHA-256 отпечаток серверного сертификата, отдаётся как `pinSHA256` в URI — чтобы **самоподписанный** сертификат проверялся без публичного CA, не светя SNI-домен в логах Certificate Transparency и оставаясь защищённым от MITM. Пусто = обычное доверие к CA. |
| `in_main_sub` | При `true` персональный `hysteria2://` URI также добавляется в link-подписку (`/sub/{token}/links`, `/c/{token}/links`, `?format=links`/`b64`) — клиенты получают резерв как дополнительный сервер, не импортируя его отдельно. URI hy2 **добавляется, а не заменяет** основные ссылки. |

Эндпоинты:

| Эндпоинт | Авторизация | Назначение |
|---|---|---|
| `GET /sub/{token}/hy2` (+ `.b64`) | токен подписки | Возвращает персональный клиентский URI `hysteria2://`. |
| `GET /api/users/{id}/hy2` | admin-токен | Прочитать флаг `hy2_enabled` пользователя. |
| `PUT /api/users/{id}/hy2` | admin-токен | Переключить доступ пользователя к резерву (`{"enabled": true}`). |
| `POST /hysteria/auth` | только loopback | Auth-backend, который нативный hysteria-демон (`auth.type: http`) вызывает для проверки каждого подключения по БД — даёт per-user отзыв и per-user id для статистики. |

Это работает в паре с ролями `hysteria` + relay из [Raven-server-install](https://github.com/AlchemyLink/Raven-server-install) (нативные hysteria-серверы достижимы через UDP-форвард на RU nginx поверх WireGuard-меша).
